...
import { COMMERCIAL_STATUSES } from '@/lib/constants'
import { getPlanCatalogItem } from '@/lib/plan-catalog'
import { useCrmStore } from '@/store/crm-store'
import type { Client, CommercialStatus } from '@/types/domain'
  ...
  const filtered = useMemo(
    () =>
      clients.filter((client) => {
        if (client.estadoComercial === 'Cerrado perdido') return false
        if (responsable !== 'all' && client.responsablePrincipal !== responsable) return false
        const search = [client.nombreCliente, client.plan, client.estadoComercial, client.proximaAccion ?? ''].join(' ').toLowerCase()
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Vista Kanban para leads, demos, propuestas y cierres. Arrastrá tarjetas, filtrá por responsable y abrí la ficha sin salir del flujo."
      />
      <div className="grid gap-4 md:grid-cols-3">
        <MetricCard title="Leads" value={filtered.filter((client) => client.estadoComercial === 'Lead').length} />
        <MetricCard title="Demos" value={filtered.filter((client) => ['Demo agendada', 'Demo realizada'].includes(client.estadoComercial)).length} />
        <MetricCard title="Propuestas" value={filtered.filter((client) => client.estadoComercial === 'Propuesta enviada').length} />
      </div>
      <Card>
        <CardContent className="grid gap-3 p-4 md:grid-cols-[minmax(0,1fr)_220px]">
          <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar cliente, plan o próxima acción..." />
      ...
      <PipelineBoard
        ...
        onMove={(clientId, status) => {
          ...
          if (!client) return
          upsertClient({ ...client, estadoComercial: status as CommercialStatus, updatedAt: new Date().toISOString() })
          toast.success(`"${client.nombreCliente}" movido a ${status}`)
        }}
      />
